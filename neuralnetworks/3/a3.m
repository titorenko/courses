...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % data.outputs is a matrix of size <number of classes i.e. 10> by <number of data cases>
  ...
  nTrainingCases = size(data.inputs, 2);
  %fprintf("hello %d",nTrainingCases);
  ...
  d_E_dzj = class_output - data.targets;  % c by t
  
  d_C_wij = zeros(size(model.hid_to_class));
  for tCase = 1:nTrainingCases
	d_E_wij_t = hid_output(:, tCase)*d_E_dzj(:, tCase)'; 
	d_C_wij_t = d_E_wij_t' + wd_coefficient * model.hid_to_class;
	d_C_wij = d_C_wij + d_C_wij_t;
  end
  
  ret.hid_to_class = d_C_wij ./ nTrainingCases;

  d_C_vij = zeros(size(model.input_to_hid));
  for tCase = 1:nTrainingCases	
	dE_dz_i = model.hid_to_class' * d_E_dzj(:, tCase) .* hid_output(:, tCase) .* (1 - hid_output(:, tCase)); % h x 1  
	dE_dv_ij = dE_dz_i * data.inputs(:, tCase)';
	dC_dvij_t = dE_dv_ij + wd_coefficient * model.input_to_hid;
	d_C_vij = d_C_vij + dC_dvij_t;
  end
  
  ret.input_to_hid = d_C_vij ./ nTrainingCases;
